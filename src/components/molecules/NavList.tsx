...
export function NavList() {
  ...
  return (
    <div>
      <List sx={{ width: '100%', paddingBottom: '20px', maxWidth: 360, ...NavStyle }} component="nav">
        {combinedLinks.map((item, index) => (
          <NavLinks key={index} route={item.path} label={item.label} onClick={() => navigate(item.path)} />
        ))}
      </List>

      <List sx={{ width: '100%', height: '100%', maxWidth: 360, ...NavStyle }} component="nav">
        {secondNavLinks}
      </List>
    </div>
  );
